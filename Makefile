...
# Copyright (c) 2021 Roman Rüttimann <roman.ruettimann@gmail.com>
#
...
ifeq ($(user),)
# USER retrieved from env, GID,UID and GROUP from shell.
HOST_GROUP ?= $(strip $(if $(shell id -ng),$(shell id -ng),4000))
...
HOST_GROUP = $(strip $(if $(group),$(group),0))
HOST_GID = $(strip $(if $(uid),$(uid),0))
HOST_USER = $(user)
